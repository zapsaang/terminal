omo_switch() {
    ...
    local config_path="${base_dir}/oh-my-opencode.json"
    
    if [ -z "$suffix" ]; then
        if command -v fzf >/dev/null 2>&1; then
            suffix=$(ls "${base_dir}"/oh-my-opencode.json.bak.* 2>/dev/null | \
                     sed "s|${base_dir}/oh-my-opencode.json.bak.||" | fzf --height 40% --reverse --prompt="选择配置后缀: ")
        else
    ...
    local target_file="${base_dir}/oh-my-opencode.json.bak.${suffix}"
...
_omo_switch_completion() {
    local base_dir="$HOME/.config/opencode"
    local -a files
    files=($(ls ${base_dir}/oh-my-opencode.json.bak.* 2>/dev/null | sed "s|${base_dir}/oh-my-opencode.json.bak.||"))
